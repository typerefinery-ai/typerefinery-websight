...
require.config({
  paths: {
    "nbextensions/stix2viz/d3": "resources/d3/d3"
  }
});

require(["domReady!", "resources/stix2viz/stix2viz"], function (document, stix2viz) {
  ...
  function vizStixWrapper(content, customConfig) {
    cfg = {
      iconDir: "resources/stix2viz/icons"
    }
  ...
  function fetchJsonFromUrl() {
    ...
    queryStrings = parseQuery(getQueryStringAfter(url,"?"))
    hashbangStrings = parseQuery(getQueryStringAfter(url,"#"))

    urlString = queryStrings["url"] || hashbangStrings["url"]

    if (urlString != "") {

      // Fetch JSON from the url
      fetchJsonAjax(urlString, function(content) {
  ...
  function getQueryStringAfter(url,separator) {
