...
(function (ns, themeNs, clientNs, componentNs, document, window) {
  ...
  ns.tmsConnection = () => {
    const listOfHost = JSON.parse(
      localStorage.getItem("tmsHost") || '["ws://localhost:8112/$tms"]'
    );
        ...
        if (payload.data) {
          ...
          if ($component) {
            if ($component.getAttribute("data-module") === "tickerComponent") {
              componentNs?.Widgets?.Ticker?.dataReceived(
                payload.data,
                $component
              );
            } else if (
              $component.getAttribute("data-module") === "linechartComponent"
            ) {
              componentNs?.Graphs?.LineChart?.dataReceived(
                payload.data,
                $component
              );
            } else if (
              $component.getAttribute("data-module") === "piechartComponent"
            ) {
              componentNs?.Graphs?.PieChart?.dataReceived(
                payload.data,
                $component
              );
            } else if (
              $component.getAttribute("data-module") ===
              "xaxisbarchartComponent"
            ) {
              componentNs?.Graphs?.XAxisBarChart?.dataReceived(
                payload.data,
                $component
              );
            } else if (
              $component.getAttribute("data-module") === "barchartComponent"
            ) {
              componentNs?.Graphs?.BarChart?.dataReceived(
                payload.data,
                $component
              );
            } else if (
              $component.getAttribute("data-module") === "smallTickerComponent"
            ) {
              componentNs?.Widgets?.SmallTicker?.dataReceived(
                payload.data,
                $component
              );
            }
