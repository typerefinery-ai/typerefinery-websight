...
(function ($, ns, componentNs, eventNs, tmsNs, modalNs, formNs, document, window) {
    "use strict";
          ...
          if (formData) {
            ...
            ns.addComponentFormListeners($component, {
                ...
                callbackFn: ($component, data, statusMessage) => {
                    console.log(["embed form callback", $component, data, statusMessage, sourceUrl]);
                },
                callbackFnData: () => {
                    console.log(["embed form callbackFnData", $component, eventData, sourceUrl]);
                    return formData;
                }
            });
          }
    ...
    ns.componentRegisterEvent = ($component, listenerTarget, listenerType, name, handler, callback, options) => {
        ...
        //listen for global message events that are emited by iframe
        let componentId = $component.attr('id') || $component.parent().attr('id');
        let eventHandlerId = ns.generateEventControllerId(componentId, name);
        console.log(["componentId", componentId, "eventHandlerId", eventHandlerId]);
        let controller = new AbortController();
        ...
        listenerTarget.addEventListener(listenerType, ns.componentListeners.get(eventHandlerId).handler, {
            signal: ns.componentListeners.get(eventHandlerId).controller.signal
        });
    ...
    ns.componentUnregisterEvent = ($component, eventName) => {
        ...
        let eventHandlerId = ns.generateEventControllerId(componentId, eventName);
    ...
    ns.frameMessageHandler = (event) => {
        console.groupCollapsed(`embed frameMessageHandler on ${window.location}`);
        console.log(["event", event]);
        ...
        console.log(["componentListeners", ns.componentListeners, ns.componentListeners.has(event.source)]);
        ...
        try {
            let $component = eventSource?.frameElement?.closest(ns.selectorComponent);
            let componentId = $component?.id || $component?.parentElement?.id;
            console.log(["componentId", componentId]);
            let eventHandlerId = ns.generateEventControllerId(componentId, messageName);
            console.log(["eventHandlerId", eventHandlerId, ns.componentListeners.has(eventHandlerId)]);
            ...
            if (ns.componentListeners.has(eventHandlerId)) {
                let {$component, callback} = ns.componentListeners.get(eventHandlerId);
            ...
            try {
                ...
                if (frameStatus !== 200) {
                    ...
                    if (callbackFnData) {
                        ...
                        const eventPayloadData = eventNs.compileEventData(data, formNs.ACTIONS.FORM_LOAD, formNs.ACTIONS.FORM_LOAD, $component.componentId, null);
                        console.log(["send eventPayloadData to iframe", eventPayloadData]);
                        ns.sendMessageToiFrame($component, formNs.ACTIONS.FORM_LOAD, eventPayloadData);
                    }
                }
            } catch (e) {
                console.log("could not read frame status, iframe is from different origin.");
    ...
    ns.addComponentFormListeners = ($component, options) => {
        ...
        ns.componentRegisterEvent($component, 
            iframe, 
            ns.COMPONENT_LISTENER_LOAD, 
            ns.MESSAGE_NAMES.FORM_LOAD_DATA, function(event) {
                console.log(["event", event]);
                console.log("iframe loaded");
                ...
                try {
                    ...
                    if (frameStatus !== 200) {
                        ...
                        if (options.callbackFnData) {
                            ...
                            console.log(["callbackFnData", data]);
                            let componentId = $component.attr('id');
                            const eventPayloadData = eventNs.compileEventData(data, formNs.ACTIONS.FORM_LOAD, formNs.ACTIONS.FORM_LOAD, componentId, null);
                            console.log(["send eventPayloadData to iframe", eventPayloadData]);
                            ns.sendMessageToiFrame($component, formNs.ACTIONS.FORM_LOAD, eventPayloadData);
                        }
                    }
        ...
        ns.componentRegisterEvent($component, window, ns.WINDOW_LISTENER_MESSAGE, ns.MESSAGE_NAMES.FORM_SUCCESS, ns.frameMessageHandler, ($component, data, eventHandlerId) => {
            console.log("form submitted");
            console.log(["$component", $component, "data", data]);

            if (options.callbackFn) {
                options.callbackFn($component, data, ns.MESSAGE_NAMES.FORM_SUCCESS);
            }
    
        });
        ns.componentRegisterEvent($component, window, ns.WINDOW_LISTENER_MESSAGE, ns.MESSAGE_NAMES.FORM_CANCEL, ns.frameMessageHandler, ($component, data, eventHandlerId) => {
            console.log("form cancelled");
            console.log(["$component", $component, "data", data]);

            if (options.callbackFn) {
                options.callbackFn($component, data, ns.MESSAGE_NAMES.FORM_CANCEL);
            }

        });
        ns.componentRegisterEvent($component, window, ns.WINDOW_LISTENER_MESSAGE, ns.MESSAGE_NAMES.FORM_ERROR, ns.frameMessageHandler, ($component, data, eventHandlerId) => {
            console.log("form error");
            console.log(["$component", $component, "data", data]);
    
            if (options.callbackFn) {
                options.callbackFn($component, data, ns.MESSAGE_NAMES.FORM_ERROR);
            }
