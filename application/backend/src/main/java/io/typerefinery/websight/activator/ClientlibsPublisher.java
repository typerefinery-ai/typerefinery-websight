package io.typerefinery.websight.activator;
...
public class ClientlibsPublisher implements BundleListener, Runnable {
  ...
  private void publishWebRoot(String webRoot, Set<String> publishedFiles) {
    try {
      ResourceResolver resourceResolver = this.resourceResolverFactory.getServiceResourceResolver(Map.of("sling.service.subservice", "websight-webroot-publish"));
      try {
        ...
        for (Resource file : files) {
          if (!publishedFiles.contains(file.getPath())) {
            List<String> processors = this.publishService.publish(file, new PublishOptions(PublishAction.PUBLISH));
