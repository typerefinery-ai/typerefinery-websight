...
public abstract class AbstractAssetsRestAction<T extends AssetsRestModel, R> implements RestAction<T, R> {
  ...
  public RestActionResult<R> perform(T model) {
    ...
    for (String item : items) {
      ...
      if (!JcrUtil.isFolder(child) && !model.getAssetManager().isAsset(child.getPath()))
        return RestActionResult.failure(failureMessage, 
            String.format("Resource '%s' must be a folder or an asset.", new Object[] { item })); 
      resources.add(child);
    } 
    try {
