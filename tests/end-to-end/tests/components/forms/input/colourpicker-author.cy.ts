...
const paths = {
  colorInput:
    "ComponentOverlay_/content/typerefinery-showcase/pages/components/forms/input/jcr:content/rootcontainer/container/form1/container_7/input_18/field",
};

describe("Input Component - Colour Picker (Author)", () => {
  beforeEach(() => {
    cy.login();
  });

  it("renders colour picker correctly in edit mode", () => {
    cy.intercept(
      "POST",
  ...
  it("has colour picker component overlay", () => {
    cy.visit(
      "/apps/websight/index.html/content/typerefinery-showcase/pages/components/forms/input::editor"
    );

    cy.getByTestId(paths.colorInput)
      .find(selectors.overlayName)
      .should("have.text", "blockInput");
  });

  it("has editable General tab in dialog for colour picker", () => {
    cy.visit(
      "/apps/websight/index.html/content/typerefinery-showcase/pages/components/forms/input::editor"
    );

    cy.getByTestId(paths.colorInput).click();
